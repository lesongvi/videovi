...
(function($, undefined) {
	...
	jPlayerPlaylist.prototype = {
		...
		_refresh: function(instant) {
			...
			if(instant && !$.isFunction(instant)) {
				...
				$(this.cssSelector.playlist + " ul").slideUp(displayTime, function() {
					...
					var main_player = self.options.main_player;
					var jplayer_id = self.options.jplayer_id;


					if (main_player == 'main')
					{
                        $('#jp-current-time'+jplayer_id).css('display', 'block');

                        if (self.options.top_frame_q == '1')
                        {
                            $('#player_poster_wrap'+jplayer_id).css('display', 'block');
                        }

                        if (self.options.playlist_q == '1')
                        {
                            $('#jp-playlist_q'+jplayer_id).css('display', 'block');
                        }

                        if (self.options.playlist_q == '0' && self.options.top_frame_q == '0')
                        {
                            $('#player_poster_song_name_2'+jplayer_id).css('display', 'block');
                        }
					}
					else
					{
					    $('#jp-current-time_ll'+jplayer_id).css('display', 'block');
					}
				});
			}
		},
		_createListItem: function(media) {
			var self = this;
			var list_cnt = $('.pl_list_row').length + 1;
			var list_cnt_layout_arr = ["7", "8", "9", "10", "11", "12"]
			var list_cnt_text = '';

			if (list_cnt_layout_arr.indexOf(this.options.player_layout) >= 0)
			{
				list_cnt_text = list_cnt +'. ';
			}

            var row_class_name = $('.pl_list_row').length % 2 ? 'player_row_odd' : 'player_row_even';
			// Wrap the <li> contents in a <div>
			var listItem = "<li class='pl_list_row "+row_class_name+"'><div>";

			// Create remove control
			listItem += "<div class='" + this.options.playlistOptions.removeItemClass + "'>&times;</div>";
			...
			if(media.free) {
				...
				$.each(media, function(property,value) {
					if($.jPlayer.prototype.format[property]) { // Check property is a media format.
						...
						listItem += "<div class='" + self.options.playlistOptions.freeItemClass + "' href='" + value + "' tabindex='1'>" + property + "</div>";
					}
				});
				listItem += ")</span>";
			}
			...
			listItem += "<div class='" + this.options.playlistOptions.itemClass + "' tabindex='1' onclick='cl_f();'>"+ list_cnt_text + media.title + (media.artist ? " <span class='jp-artist'>by " + media.artist + "</span>" : "") + "</div>";
			listItem += "</div></li>";
		...
		_createItemHandlers: function() {
			...
			$(this.cssSelector.playlist + " div." + this.options.playlistOptions.itemClass).die("click").live("click", function() {
			...
			$(self.cssSelector.playlist + " div." + this.options.playlistOptions.freeItemClass).die("click").live("click", function() {
			...
			$(self.cssSelector.playlist + " div." + this.options.playlistOptions.removeItemClass).die("click").live("click", function() {
				var index = $(this).parent().parent().index();
				self.remove(index);
				$(this).blur();
				return false;
		...
		select: function(index) {
			...
			var track_name = this.playlist[index].title;
			var album_name = this.playlist[index].album;
			var track_thumb = this.playlist[index].poster;
			var song_id = this.playlist[index].song_id;
			var itunes_url = this.playlist[index].itunes;
			var amazon_url = this.playlist[index].amazon;
			var cd_baby_url = this.playlist[index].cd_baby;
			var main_player = this.options.main_player;
			var jplayer_id = this.options.jplayer_id;
			var s_netw = this.options.s_netw;
			var s_commerce = this.options.s_commerce;
			var player_layout = this.options.player_layout;
			var pl_layout_img_url = player_layout+'/';

			if (player_layout === undefined)
			{
				pl_layout_img_url = '';
			}
			...
			if (main_player == 'main')
			{
                document.getElementById('player_poster_song_name'+jplayer_id).innerHTML = track_name + '&nbsp;';
                document.getElementById('player_poster_song_name_2'+jplayer_id).innerHTML = track_name + '&nbsp;';
                document.getElementById('player_poster_album_name'+jplayer_id).innerHTML = album_name + '&nbsp;';
                document.getElementById('player_poster_im'+jplayer_id).innerHTML = '<img src="'+track_thumb+'" alt="" />';
		    }


		    document.getElementById('links_wrap'+jplayer_id).innerHTML =    '<div id="itunes_link'+jplayer_id+'" class="mp3_player_commerce_link"></div>' +
		                                                                    '<div id="amazon_link'+jplayer_id+'" class="mp3_player_commerce_link"></div>' +
		                                                                    '<div id="cd_baby_link'+jplayer_id+'" class="mp3_player_commerce_link"></div>' +
		                                                                    '<div id="share_links'+jplayer_id+'" class="mp3_player_share_links"></div>';

		    //social network links START
            var fb_link = s_netw == '1' ? '<img src="/js/s101_admin/mp3_player/img/'+pl_layout_img_url+'fb_sm.png" alt="" class="mp3_player_commerce_link" onclick="mp3_shearer(\'facebook\', \''+song_id+'\');" />' : '';
            var tw_link = s_netw == '1' ? '<img src="/js/s101_admin/mp3_player/img/'+pl_layout_img_url+'tw_sm.png" alt="" class="mp3_player_commerce_link" onclick="mp3_shearer(\'twitter\', \''+song_id+'\');" />' : '';

            document.getElementById('share_links'+jplayer_id).innerHTML = fb_link + tw_link;
		    //social network links START
		    ...
		    if (s_commerce == '1')
		    {
                if (itunes_url.length >= 3)
                {
                    document.getElementById('itunes_link'+jplayer_id).innerHTML = '<img src="/js/s101_admin/mp3_player/img/'+pl_layout_img_url+'apple.png" alt="" onclick="window.open(\''+itunes_url+'\', \'buy_on_itunes\');" />';
                }
                if (amazon_url.length >= 3)
                {
                    document.getElementById('amazon_link'+jplayer_id).innerHTML = '<img src="/js/s101_admin/mp3_player/img/'+pl_layout_img_url+'amazon.png" alt="" onclick="window.open(\''+amazon_url+'\', \'buy_on_amazon\');" />';
                }
                if (cd_baby_url.length >= 3)
                {
                    document.getElementById('cd_baby_link'+jplayer_id).innerHTML = '<img src="/js/s101_admin/mp3_player/img/'+pl_layout_img_url+'cd_baby.png" alt="" onclick="window.open(\''+cd_baby_url+'\', \'buy_on_cd_baby\');" />';
                }
            }
		    //ecommerce links END
		...
		shuffle: function(shuffled, playNow) {
			...
			if(shuffled || shuffled !== this.shuffled) {
				...
				$(this.cssSelector.playlist + " ul").slideUp(this.options.playlistOptions.shuffleTime, function() {
					...
					$('#jp-playlist_q').css('display', 'block');
				});
			}
		}
	};
...
function volume_do(player_id, opt_switch) {

	var cur_volume = $('#'+player_id).data("jPlayer").options.volume;
	var new_volume = 0;

	if (opt_switch == "-")
	{
		new_volume = cur_volume - 0.30;
	...
	$('#'+player_id).jPlayer({volume:new_volume});
}


function mp3_shearer(network, song_id) {

    var wind_width;
    var wind_height;
	var site_url = 'http://'+ window.location.href.split('/')[2];

	if (network == 'twitter')
	{
		wind_width = 600;
    	wind_height = 400;
    	link_url = 'https://twitter.com/share?url='+encodeURIComponent(site_url+'/song&song_id='+song_id);
	}
	else if (network == 'facebook')
	{
		wind_width = 1000;
    	wind_height = 600;
    	link_url = 'http://www.facebook.com/sharer.php?u='+encodeURIComponent(site_url+'/song&song_id='+song_id);
	}
    ...
    var network_window = window.open(link_url, 'share_window', wind_params);
}
